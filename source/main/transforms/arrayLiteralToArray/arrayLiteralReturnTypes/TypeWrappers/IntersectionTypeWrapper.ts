...
export class IntersectionTypeWrapper extends TypeWrapper {
    constructor(protected readonly type: Type, protected readonly sourceFile: SourceFile) {
        super(type, sourceFile);
    }

    public getText(): string {
        return this.type.getIntersectionTypes().map((childType) => {
            return this.buildTypeText(childType);
        }).join(" & ");
    }

    public getTypeTree(): TypeTree {
        const childTrees = this.type.getIntersectionTypes().map((childType) => {
